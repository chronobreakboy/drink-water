...
<style>
:root{
  --sky-1:#ffd8ec; --sky-2:#ffcfe0; --sky-3:#ffe7c2;
  --card:#fff7de; --stroke:#f2d278; --text:#5b402f; --muted:#6f5b51;
  --btn-grad:linear-gradient(180deg,#ffd0e0,#ffb1c4);
  --btn-grad-2:linear-gradient(180deg,#ffe1b8,#ffd59f);
  --shadow:0 14px 36px rgba(255,140,180,.30); --radius:20px;
}
...
.clouds,.clouds:before,.clouds:after{
  position:absolute;left:-20%;right:-20%;top:0;bottom:0;content:"";
  background:
    radial-gradient(120px 60px at 10% 20%, #fff8 40%, transparent 60%),
    radial-gradient(160px 70px at 30% 25%, #fff9 40%, transparent 62%),
    radial-gradient(140px 60px at 55% 22%, #fff8 40%, transparent 60%),
    radial-gradient(180px 80px at 75% 28%, #fff7 40%, transparent 62%),
    radial-gradient(140px 60px at 90% 20%, #fff8 40%, transparent 60%);
  filter:blur(6px); animation:drift 48s linear infinite; opacity:.7;
}
...
.star{
  ...
  opacity:.85;pointer-events:none;z-index:1;animation:twinkle linear infinite;
}
...
.progress-label{font-size:13px;color:#6f5b51;margin-bottom:8px}
...
.small{font-size:12px;text-align:center;color:#6f5b51;margin-top:6px}
...
st.markdown("<div class='sky'>"+stars_html+"</div>", unsafe_allow_html=True)
...
# ================== JS PERSISTENTE (rebind + iOS unlock + áudio + partículas) ==================
js_template = r"""
<script>
(function(){
  ...
  function ensureAC(forceNew=false){
    if(forceNew || !window._ac){
      const AC = window.AudioContext || window.webkitAudioContext;
      try { window._ac = new AC(); } catch(e){ window._ac = null; }
    }
  ...
  function unlockAudioOnce(){
    ...
    const ac = ensureAC(true);
    if(!ac) return;
    try{
      // toca 1 sample de silêncio
  ...
  // --- PARTÍCULAS ---
  function burstNow(){
    document.querySelectorAll('.flash,.burst').forEach(e=>e.remove());
  ...
  function bindSound(){
    ...
    if(btn && !btn.dataset.soundBound){
      const handler = ()=>{ playYay(); burstNow(); };
  ...
  setInterval(()=>{ styleButtons(); bindSound(); }, 350);
})();
</script>
"""
...
st.markdown(
    ...
    f"<div class='progress-label'>{total} / {goal_ml} ml ({pct}%) — cada copinho: {cup_ml} ml</div>"
...
# ================== EFEITO VISUAL + AUTOPLAY NO RERUN (reforço) ==================
if st.session_state.get("do_effect"):
    ...
    # tenta tocar também no rerun (desktop ajuda)
    if YAY_B64:
        st.components.v1.html(
            f"<!DOCTYPE html><html><head><meta charset='utf-8'></head><body>"
            f"<audio autoplay playsinline><source src='data:audio/mpeg;base64,{YAY_B64}' type='audio/mpeg'></audio>"
            f"</body></html>",
